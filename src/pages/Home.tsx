...
const Home: React.FC = () => {
    ...
    const nigerianNames = [
        'Aisha Bello', 'Chinedu Okoro', 'Fatima Abubakar', 'Gbenga Adekunle', 'Ngozi Eze',
        'Kunle Ojo', 'Amaka Ndubuisi', 'Tunde Olalekan', 'Zainab Musa', 'Emeka Obi',
        'Damilola Femi', 'Hauwa Ibrahim', 'Kelechi Nwosu', 'Sola Alabi', 'Aminu Umar',
        'Precious Onyeka', 'David Effiong', 'Blessing Godwin', 'Segun Dada', 'Yemi Adewale'
    ];

    // --- useEffect Hooks for dynamic UI elements ---
    useEffect(() => {
        const interval = setInterval(() => {
            setWatchingCount(prevCount => prevCount + Math.floor(Math.random() * 5) + 1);
        }, 3000);
    ...
    useEffect(() => {
        const checkPaystack = setInterval(() => {
            // Check if window.PaystackPop exists AND if its 'setup' method exists and is a function
    ...
    const bonuses: Bonus[] = [
        ...
        },
        {
            ...
            description: 'Discover cheap and effective feed formulations to grow fish bigger, faster',
        },
        {
            title: 'Access to supplies & buyers',
            description: 'Get trusted contacts for supplies and eager buyers instantly',
        },
        {
            title: 'Support group access',
            description: 'Join a vibrant WhatsApp community of successful Catfish hatchers for ongoing support',
        },
    ];
    ...
    const handlePaystackCheckout = (): void => {
        ...
        // Robust check: ensure PaystackPop and its setup method are available
        if (typeof window.PaystackPop === 'undefined' || typeof window.PaystackPop.setup !== 'function') {
            console.error('PaystackPop or its setup method is not defined. Ensure Paystack script is loaded in index.html and accessible.');
            setIsLoading(false);
            alert('Payment system is still loading. Please try again in a moment.');
            return;
        }

        // Initialize Paystack payment (using the setup method directly, no 'new')
        const handler = window.PaystackPop.setup({
            key: paystackPublicKey,
            email: email || 'customer@example.com', // Use provided email, fallback to placeholder
            amount: 200000, // Amount in kobo (₦2,000.00)
            currency: 'NGN',
            ref: 'catfish_course_' + Math.floor(Math.random() * 1000000000 + 1), // Unique reference
            metadata: {
                custom_fields: [
                    {
            ...
            callback: function (response) {
                ...
                setIsLoading(false);
                setSpotsLeft(prev => Math.max(0, prev - 1)); // Decrement spots on success
                setShowPaymentModal(false); // Close modal
                navigate('/access-course'); // Redirect to access page
            },
            onClose: function () {
    ...
    const handleTransferPayment = (): void => {
        ...
        const whatsappLink = `https://wa.link/531kt2?text=${encodeURIComponent(whatsappMessage)}`; // Replace with your actual WhatsApp number
        ...
        <div className='min-h-screen bg-gradient-to-br from-white via-gray-50 to-blue-50 text-gray-900'>
                ...
                <div className='fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center p-4'>
                    <div className='relative w-full max-w-4xl aspect-video bg-black rounded-xl overflow-hidden shadow-2xl'>
                        ...
                        <iframe
                            ...
                            src='https://www.youtube.com/embed/Fwqw5mST9OY?si=DbcQcsEX3RVIZg3j' // Example YouTube URL
            ...
            {showPaymentModal && (
                <div className='fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4'>
                    <div className='bg-white rounded-xl p-8 shadow-2xl max-w-md w-full relative'>
                        ...
                        <h2 className='text-2xl font-bold mb-6 text-center text-red-600'>Choose Payment Method</h2>
                        <div className='space-y-4'>
                            <div className='bg-blue-50 border border-blue-200 rounded-lg p-4'>
                                <h3 className='font-semibold text-lg text-blue-700 mb-2'>Direct Transfer</h3>
                                <p className='text-gray-700 mb-3'>
                                    Bank Name: MONIEPOINT MFB<br />
                                    Account Number: 8060483871<br />
                                ...
                                <p className='text-sm text-gray-600 mt-2'>
                                    (After transfer, click to send receipt and email for confirmation)
                                </p>
                            </div>
                            <div className='bg-green-50 border border-green-200 rounded-lg p-4'>
                                <h3 className='font-semibold text-lg text-green-700 mb-2'>Pay Online with Paystack</h3>
                                <button
                                    ...
                                    {isLoading ? 'Processing...' : isPaystackLoaded ? 'Pay with Paystack' : 'Loading payment system...'}
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            {notification && (
                <div className='fixed top-4 right-4 bg-green-500 text-white p-4 rounded-lg shadow-lg z-50 animate-fadeInOut'>
                    <p className='font-bold'>{notification.name} just paid ₦{notification.amount.toLocaleString()} now!</p>
                </div>
            )}
                ...
                <div className='bg-white rounded-xl shadow-lg p-8 border-2 border-red-200'>
                    ...
                    <div className='text-center'>
                        <button
                            ...
                            disabled={isLoading || !email || !isPaystackLoaded}
                            ...
                            {isLoading ? '⏳ Processing...' : !isPaystackLoaded ? 'Loading payment system...' : 'Click to Pay'}
                        </button>
                        ...
                        {!isPaystackLoaded && email && !isLoading && (
